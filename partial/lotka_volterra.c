/*
lotka_volterra.c (v7) - fixed bug in log_sum_in!!!
...
Added fix for sticky intervals...
...
int main(int argc,char *argv[])
{
  ...
  gsl_rng_set(rng,1); /* set a seed here... */
...
void update_in(interval *in_prop0,gsl_vector_int *type_vec0,interval *in_prop1,gsl_vector_int *type_vec1)
{
...
#ifdef UPD_START
  gsl_vector_int_set(sticky,0,gsl_vector_int_get(sticky,0)+1);
  ...
  if ( (r0>=0) && (r1>=0) && (r2>=0) && (y1s>=0) ) {
	...
	-log_pi(r2o,c2*(y1+y1n)/2.0);
      if ( accept_lp(a) || (gsl_vector_int_get(sticky,0)>1000) ) {
	interval_cpy(in[0],in_prop0);
	gsl_matrix_int_set(data,0,1,y1s);
	gsl_vector_int_set(sticky,0,0);
      }
    }
  }
#endif
